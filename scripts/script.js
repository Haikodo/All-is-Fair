document.addEventListener("DOMContentLoaded", function () {
  ...
  class NPC {
    ...
    getIntroduction() {
      return `${
        this.name
      }, a ${this.background}, seems ${this.personality.toLowerCase()}.`;
    }
  ...
  class Relationship {
    ...
    getOpinionsWithNames(npcRegistry) {
      return Object.keys(this.npcOpinions).map((tokenId) => {
        return {
          name: npcRegistry[tokenId]
            ? npcRegistry[tokenId].name
            : "Unknown NPC",
  ...
  function saveNPCs(slot = currentSaveSlot) {
    const npcData = npcs.map((npc) => {
      const memoryCopy = {
        ...
        Events: Array.isArray(npc.memory.Events)
          ? npc.memory.Events.map((event) => ({ ...event }))
  ...
  function loadNPCs(slot = currentSaveSlot) {
    ...
    if (storedNPCs) {
      npcs = JSON.parse(storedNPCs).map((npcData) => {
        ...
        npc.memory.Relationships.opinionPlayer =
          npcData.memory.Relationships.opinionPlayer;
        ...
        npc.memory.Relationships.npcOpinions =
          npcData.memory.Relationships.npcOpinions;
  ...
  function setStance(stance) {
    let responseLog = {
      positive:
        "You console the survivor, reassuring them that they did what they could.",
    ...
    console.log(
      ...
      npcs.map((npc) => ({
        ...
        memory: {
          ...
          friends: npc.memory.Relationships.friends.map((friendId) =>
            npcRegistry[friendId] ? npcRegistry[friendId].name : "Unknown NPC"
          ),
          enemies: npc.memory.Relationships.enemies.map((enemyId) =>
            npcRegistry[enemyId] ? npcRegistry[enemyId].name : "Unknown NPC"
          ),
        },
      }))
  ...
  function addOrUpdateNPC(npc) {
    ...
    const existingNPCIndex = npcs.findIndex(
      (existingNPC) => existingNPC.tokenId === npc.tokenId
    );
        ...
        if (Array.isArray(importedData)) {
          importedData.forEach((npcData) => {
            if (validateNPCData(npcData)) {
              let existingNPCIndex = npcs.findIndex(
                (npc) => npc.tokenId === npcData.tokenId
              );
              let newNPC = new NPC(
                npcData.name,
                npcData.personality,
              ...
              if (existingNPCIndex !== -1) {
                ...
                console.log(
                  `${npcData.name} with tokenId ${npcData.tokenId} has been overwritten.`
                );
              } else {
                // Add new NPC
                npcs.push(newNPC);
  ...
  function switchToMapMode() {
    battleState = "map";
    battleLog.innerHTML =
      "You are in Map mode. Select a battlefield to start the battle.";
  ...
  function statusCheck() {
    try {
      ...
      // Find and remove any NPC with the property "testNPC"
      npcs = npcs.filter((npc) => {
        if (npc.property.includes("testNPC")) {
          delete npcRegistry[npc.tokenId];
          console.log(`%cRemoved test NPC: ${npc.name}`, "color: green;");
          return false; // Remove from npcs array
        }
        return true; // Keep in npcs array
      });
      ...
      console.log(
        "%cStatus check completed successfully.",
